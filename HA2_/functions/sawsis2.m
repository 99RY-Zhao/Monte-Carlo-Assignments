...
function [X,Y,W] = sawsis2(n,k,g,resampling)
    if g == "naive"
        g = @g_naive;
    elseif g == "smart"
        g = @g_smart;
    ...
    for step=2:k
        for particle=1:n
            ...
            [Xnew,Ynew,Pnew] = g(X,Y,step,particle);

            % No point in continuing if we crashed.
            fail = nnz((X(1:step-1,particle) == Xnew).*(Y(1:step-1,particle) == Ynew));
            if fail == 0
                % Update the values
                X(step,particle) = Xnew;
                Y(step,particle) = Ynew;
                
                % Update the weights. If we run SISR the previous weight is
                % going to be one and the algorithm works the same.
...
function [Xnew,Ynew,Pnew] = g_naive(X,Y,step,particle)
...
function [Xnew,Ynew,Pnew] = g_smart(X,Y,step,particle)
    ...
    for i=1:4
        ...
        fail = nnz((X(1:step-1,particle) == Xtest).*(Y(1:step-1,particle) == Ytest));
        if fail == 0
